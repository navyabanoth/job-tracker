  ...
  return (
    <div className="min-h-screen gradient-bg grid place-items-center px-4 sm:px-6 py-8">
      <div className="w-full max-w-md bg-white/90 backdrop-blur rounded-2xl p-6 shadow-card border border-black/5">
        ...
        <p className="text-sm text-black/60 mb-6">Join and track your dream jobs with focus.</p>
        <form onSubmit={onSubmit} className="space-y-4">
          <input className="w-full rounded-xl border border-black/10 px-4 py-2" placeholder="Full name" value={name} onChange={(e)=>setName(e.target.value)} required />
          ...
          <button className="w-full rounded-xl bg-primary-600 text-white px-4 py-2 font-medium shadow-card hover:bg-primary-700">Sign up</button>
        </form>
        <p className="text-sm text-black/60 mt-4">Already have an account? <Link to="/login" className="text-primary-700 font-medium">Log in</Link></p>
      </div>
    </div>
  );
}
